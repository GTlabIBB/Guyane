...
zotus <- read.table("classification_sickel/zotus.sintax.txt", header=F, sep="\t", fill = TRUE)
...
zotus$V2 <- data.frame(do.call('rbind', strsplit(as.character(zotus$V2),',f:',fixed=TRUE)))$X2
...
f <- data.frame(do.call('rbind', strsplit(gsub(")", "",classif_raw$X1),'(',fixed=TRUE)))
f$X1 <- gsub("f:", "", f$X1)
f$X2 <- gsub("f:", "", f$X2)
f$X2 <- as.numeric(f$X2)
g <- data.frame(do.call('rbind', strsplit(gsub(")", "",classif_raw$X2),'(',fixed=TRUE)))
g$X1 <- gsub("g:", "", g$X1)
g$X2 <- gsub("g:", "", g$X2)
g$X2 <- as.numeric(g$X2)
s <- data.frame(do.call('rbind', strsplit(gsub(")", "",classif_raw$X3),'(',fixed=TRUE)))
s$X1 <- gsub("s:", "", s$X1)
s$X2 <- gsub("s:", "", s$X2)
...
#save as xlsx file
write.xlsx(aggregated_filtered_species, "classification_sickel/zotus_aggregated_species95.xlsx", row.names=F)
write.xlsx(aggregated, "classification_sickel/zotus_aggregated_probabilities.xlsx", row.names=F)
